...
export default function BloodBankClient({ initialDonors }: { initialDonors: User[] }) {
    ...
    const thana = searchParams.get('thana') || 'all';
    ...
    /**
     * Masks the email to protect PII in tooltips/tooltips while keeping layout truncated
     */
        ...
        <div className="container py-8 max-w-7xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-muted/30 p-4 rounded-lg border">
                <div className="space-y-2">
                    ...
                    <Select value={bloodGroup} onValueChange={(val) => handleFilterChange('bloodGroup', val)}>
                ...
                <div className="space-y-2">
                    ...
                    <Select value={district} onValueChange={(val) => handleFilterChange('district', val)}>
                        ...
                        <SelectContent>
                            ...
                            {bdLocations.flatMap((div) => div.districts).sort((a, b) => a.name.localeCompare(b.name)).map((d) => (
                ...
                <div className="space-y-2">
                    ...
                    <Select value={thana} onValueChange={(val) => handleFilterChange('thana', val)} disabled={!selectedDistrictData}>
                        ...
                        <Card key={user._id} className="overflow-hidden flex flex-col h-full hover:shadow-lg transition-shadow border-muted">
                            <CardHeader className="p-0">
                                <div className="h-20 bg-gradient-to-r from-red-50 to-red-100 dark:from-red-950/20 dark:to-red-900/10 relative">
                                    <Badge className="absolute top-3 right-3 text-lg font-bold px-3 py-1 shadow-sm" variant="destructive">{user.bloodGroup}</Badge>
                                </div>
                            </CardHeader>
                            <CardContent className="flex flex-col items-center -mt-10 p-6 pt-0 flex-1">
                                ...
                                <div className="text-center mt-3 space-y-1 mb-4 w-full">
                                    ...
                                    {user.title && <Badge variant="outline" className="text-xs font-normal border-primary/20 bg-primary/5 text-primary">{user.title}</Badge>}
                                </div>
                                ...
                                <div className="w-full space-y-4 mt-auto">
                                    <div className="flex items-start gap-2.5 text-sm text-muted-foreground bg-muted/50 p-3 rounded-md">
                                        ...
                                        <span className="text-left font-medium text-foreground/80 leading-snug">
                                            {[user.address?.present?.thana, user.address?.present?.district].filter(Boolean).join(", ") || "Location unavailable"}
                                        </span>
                                    </div>
                                        ...
                                        <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300 border rounded-md p-3 bg-secondary/10">
                                            {user.email && (
                                                <div className="flex items-center gap-2 text-sm overflow-hidden">
                                                    ...
                                                    <span className="truncate" title={maskEmail(user.email)}>{user.email}</span>
                                                </div>
                                            )}
                                            {(user.whatsappNumber || user.phone) && <div className="flex items-center gap-2 text-sm"><Phone className="h-4 w-4 shrink-0 text-primary" /><span className="font-medium">{user.whatsappNumber || user.phone}</span></div>}
                                        </div>
                                    )}
                                    <Button variant={revealedContacts.has(user._id) ? "secondary" : "default"} className="w-full shadow-sm" onClick={() => toggleContact(user._id)}>
