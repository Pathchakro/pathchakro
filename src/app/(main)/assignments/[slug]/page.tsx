...
import { FileText, Calendar, Award, Users, ArrowLeft, CheckCircle, Clock, XCircle, Loader2 } from 'lucide-react';
...
interface Assignment {
    ...
    submissions: Array<{
        ...
        feedback?: string;
        status: string;
        userId?: string; // Assuming the API populates this or it's inside student
    }>;
    status: string;
    createdAt: string;
}
...
export default function AssignmentDetailPage() {
    ...
    const assignmentId = params.slug as string;
    ...
    const fetchAssignment = async () => {
        try {
            ...
            setError(error.message || 'An error occurred while loading the assignment');
            setAssignment(null);
        } finally {
            setLoading(false);
        }
    ...
    const handleGrade = async (submissionId: string) => {
        ...
        try {
            ...
            if (response.ok) {
                toast.success(data.message || 'Grade submitted');
    ...
    if (loading) {
        return (
            <div className="max-w-5xl mx-auto p-4 flex justify-center items-center min-h-[50vh]">
    ...
    // Filter submissions to find current user's submission
    const mySubmission = assignment.submissions.find(
        sub => sub.student._id === session?.user?.id || (sub as any).userId === session?.user?.id
    );
                                ...
                                {submission.grade !== undefined ? (
                                    ...
                                    <div className="space-y-3 border-t pt-4">
                                        ...
                                        <div className="flex gap-2">
                                            <Button onClick={() => handleGrade(submission._id)} className="flex-1" disabled={isGrading}>
                                                {isGrading ? (
                                                    <>
                                                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                                        Grading...
                                    ...
                                    <Button
                                        ...
                                        variant="outline"
                                        size="sm"
                                        disabled={isGrading || (gradingSubmissionId !== null && gradingSubmissionId !== submission._id)}
                                    >
                                        Grade Submission
                                    </Button>
                                )}
