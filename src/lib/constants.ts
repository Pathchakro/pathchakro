export const INTERESTS_LIST = [
    "বাংলাদেশ",
    "আন্তর্জাতিক",
    "রাজনীতি",
    "অর্থনীতি",
    "ভূগোল",
    "খেলাধুলা",
    "বিনোদন",
    "মুভি রিভিউ",
    "ভ্রমণ কাহিনী",
    "স্বাস্থ্য",
    "ইতিহাস",
    "সাহিত্য",
    "দর্শন",
    "বিজ্ঞান ও প্রযুক্তি",
    "ধর্মীয় শিক্ষা",
    "কবিতা",
    "গল্প /অনুগল্প",
    "শিশুসাহিত্য/ছড়া",
    "রম্যরচনা",
    "প্রবন্ধ",
    "আত্মজীবনী",
    "আত্মউন্নয়নমূলক",
    "ফিকশন",
    "নন-ফিকশন"

];

export const BOOK_CATEGORIES = [
    ...INTERESTS_LIST,
    "অন্যান্য"
];



export const WRITERS_LIST = [
    "অজয় সরকার",
    "অন্তিক মাহমুদ",
    "অনীশ দাস অপু",
    "অক্ষরপত্র প্রকাশনী",
    "আইজ্যাক আসিমভ",
    "আকবর আলি খান",
    "আগাথা ক্রিস্টি",
    "আচাযর্ প্রফুল্ল চন্দ্র রায়",
    "আড়ং",
    "আডলফ হিটলার",
    "আদিব সালেহ",
    "আনিসুল হক",
    "আবদুল গাফফার রনি",
    "আবদুল হাই মুহাম্মদ সাইফুল্লাহ",
    "আব্দুল কাইয়ুম",
    "আব্দুল হাসনাত আব্দুল্লাহ",
    "আব্দুল্লাহ আল মাসউদ",
    "আব্দুল্লাহ আল মাহমুদ",
    "আব্দুল্লাহ আল মামুন",
    "আব্দুল্লাহ আল মামুন (কাইকর)",
    "আব্দুল্লাহ আবু সায়ীদ",
    "আব্দুল্লাহ ইবনে মাহমুদ",
    "আব্দুল্লাহ বিন আব্দুর রাযযাক",
    "আব্দুল্লাহ মাহমুদ নজীব",
    "আব্দুর রাযযাক বিন ইউসুফ",
    "আবুল আসাদ",
    "আবুল মনসুর আহমদ",
    "আমিনুল হক",
    "আয়মান সাদিক",
    "আরিফ আজাদ",
    "আরিফ রহমান",
    "আরিফুল ইসলাম",
    "আর্নেস্ট হেমিংওয়ে",
    "আল ফাতাহ পাবলিকেশন্স",
    "আল মাহমুদ",
    "আলবার্ট আইনস্টাইন",
    "আলতাফ পারভেজ",
    "আলী আবদুল্লাহ",
    "আলী হাসান উসামা",
    "আল্লামা ইবনে কাছীর (রহ.)",
    "আল্লামা মুহাম্মদ নাসীরুদ্দীন আলবানী (রহঃ)",
    "আল্লামা সফিউর রহমান মুবারকপুরী (রহ.)",
    "আশরাফ আহমেদ",
    "আশীব ফেরদৌস অংকন",
    "আসিফ আদনান",
    "আসিফ নজরুল",
    "আহমদ ছফা",
    "আহমেদ জাওয়াদ চৌধুরী",
    "আহসান হাবীব (কার্টুনিস্ট)",
    "ইথার আখতারুজ্জামান",
    "ইফতেখার সিফাত",
    "ইমরান নযর হোসেন",
    "ইমরান রাইহান",
    "ইমরান হোসাইন আদিব",
    "ইমাম আহমাদ ইবনু হাম্বাল (রহিমাহুল্লাহ)",
    "ইমাম আবু ঈসা মুহাম্মদ ইবনে ঈসা আত তিরমিযী (রহঃ)",
    "ইমাম ইবনে তাইমিয়্যা (রা.)",
    "ইমাম ইবনু কায়্যিমিল জাওযিয়্যাহ (রহ.)",
    "ইমাম মুহিউদ্দীন ইয়াহইয়া আন-নববী (রহ.)",
    "ইমাম মুহাম্মদ ইবনে ইসমাইল বোখারী (র)",
    "ইমিন্যান্ট পাবলিকেশন্স",
    "ইমদাদুল হক মিলন",
    "ইয়াহইয়া ইউসুফ নদভী",
    "ইয়াহইয়া সিনওয়ার",
    "ইউভ্যাল নোয়া হারারি",
    "ইসমাইল আরমান",
    "ইসলামিয়া কুতুবখানা",
    "ইসলামিক ফাউন্ডেশন বাংলাদেশ",
    "ইসরাত জাহান দ্যুতি",
    "ইঞ্জি . মোঃ নূর–ই মোস্তফা",
    "ঈশিতা রহমান সানজিদা",
    "উদ্ভাস একাডেমিক এন্ড এডমিশন কেয়ার",
    "উপেন্দ্রকিশোর রায়চৌধুরী",
    "উমেরা আহমেদ",
    "উম্মে মাইসুন",
    "উইলবার স্মিথ",
    "উইলিয়াম শেক্সপীয়ার",
    "এলিফ শাফাক",
    "এস এম আকিব মুর্শেদ",
    "এস এম নাহিদ হাসান",
    "এস এম নিয়াজ মাওলা",
    "এস. এম. জাকির হুসাইন",
    "এনায়েতুল্লাহ আব্বাসি ওয়া সিদ্দিকি",
    "এনায়েতুল্লাহ আল্‌তামাশ",
    "এম. এ মোত্তালিব মিহির",
    "এম. এম. রহমত উল্লাহ্ আফনান",
    "এম. জে. বাবু",
    "এমদাদিয়া লাইব্রেরী",
    "ঐন্দ্রজল",
    "ওবায়েদ হক",
    "ওমর আল জাবির",
    "ওয়াশিংটন আরভিং",
    "ওয়ান পাবলিকেশন",
    "ওয়াসিক বিল্লাহ আসিফ",
    "ওয়াসিকা নুযহাত",
    "ওয়াহিদ তুষার",
    "ওসমান হাদি",
    "কবি নজরুল ইসলাম",
    "কবির পাবলিকেশন্স",
    "কয় কাউস",
    "করিম উদ্দিন আহমদ",
    "করিমুল্লাহ আল মামুন",
    "কাজী আজহার আলী",
    "কাজী আনোয়ার হোসেন",
    "কাজী নজরুল ইসলাম",
    "কাজী ম্যাক",
    "কাসেম বিন আবুবাকার",
    "কায় কাউস",
    "কার্ল মার্কস",
    "কিঙ্কর আহ্‌সান",
    "কেইগো হিগাশিনো",
    "কোলেন হুভার",
    "কৌশিক মজুমদার",
    "খন্দকার আল-মামুন রেজা",
    "খালেদুর রহমান কাব্য",
    "খালিদ ফারহান",
    "গাজী এস. এম. আজমল",
    "গাজী মিজানুর রহমান (বিসিএস)",
    "চমক হাসান",
    "চার্লস ডিকেন্স",
    "চেতন ভগত",
    "ছায়ামঞ্চ পাবলিকেশন্স",
    "জর্জ অরওয়েল",
    "জসীম উদদীন",
    "জাফর বিপি",
    "জাবেদ হুসেন",
    "জাভেদ পারভেজ",
    "জাভেদ হুসেন",
    "জাকির হোসাইন",
    "জাকারিয়া মাসুদ",
    "জান্নাতুল নাঈমা",
    "জাহিদ হাসান",
    "জেমস ক্লিয়ার",
    "জেমস রলিন্স",
    "জয়িতা ব্যানার্জী",
    "জহির রায়হান",
    "জীবনানন্দ দাশ",
    "জিয়াউল হক",
    "জুনায়েদ আহমেদ",
    "জুনায়েদ ইভান",
    "জুন্নুরাইন খান",
    "জুল ভার্ন",
    "জোবায়ের আল মাহমুদ",
    "জোবায়ের আহমেদ",
    "ঝংকার মাহবুব",
    "ঝিলমিল",
    "টনি মরিসন",
    "টিম বিজ্ঞান ক্লাস",
    "ড: এ. পি. জে. আবদুল কালাম",
    "ডঃ সমীর কুমার ধাড়া",
    "ড্যান ব্রাউন",
    "ড. আব্দুর রাজ্জাক বিন ইউসুফ",
    "ড. আব্দুল আযীয আব্দুর রহীম",
    "ড. আবু বকর মুহাম্মাদ যাকারিয়া",
    "ড. আবু আমিনাহ বিলাল ফিলিপ্স",
    "ড. আবুল কালাম আজাদ (বাশার)",
    "ড. আমানুল্লাহ",
    "ড. আমির হোসেন খান",
    "ড. আমিনুল ইসলাম",
    "ড. আলী মুহাম্মদ সাল্লাবি",
    "ড. আয়েয আল কারনী",
    "ড. ইয়াসির ক্বাদি",
    "ড. এ কে এম শাহনাওয়াজ",
    "ড. খোন্দকার আব্দুল্লাহ জাহাঙ্গীর",
    "ড. খালিদ আবু শাদি",
    "ড. তারেক শামসুর রেহমান",
    "ড. সৌমিত্র শেখর",
    "ড. হানান লাশিন",
    "ড. ফজলুর রহমান",
    "ড. মুহাম্মদ ইবনে আবদুর রহমান আরিফী",
    "ড. মুহাম্মদ ইউনূস",
    "ড. মুহাম্মদ ফজলুর রহমান",
    "ড. মুহম্মদ শহীদুল্লাহ",
    "ড. মিজানুর রহমান আজহারি",
    "ড. মরিস বুকাইলি",
    "ড. মোহাম্মদ অবুজর",
    "ড. মোহাম্মদ আবুল হাসান",
    "ড. মোহাম্মদ আমীন",
    "ড. মোহাম্মদ ইমাম হোসাইন",
    "ড. মোহাম্মদ মানজুরে ইলাহী",
    "ড. মোহাম্মদ সরোয়ার হোসেন",
    "ড. মোঃ শামসুল আরিফিন জিলানী",
    "ড. মোঃ শাহনেওয়াজ হোসেন",
    "ড. রাগিব সারজানি",
    "ড. শায়খ আলী তানতাবী রাহ.",
    "ড. শাহজাহান তপন",
    "ড. সরোজ কান্তি সিংহ হাজারী",
    "ডা. এস আর খান",
    "ডা. জাকির নায়েক",
    "ডা. জোসেফ মর্ফি",
    "ডা. নুসরাত জাহান দৃষ্টি",
    "ডা. মালিহা তাবাসসুম",
    "ডা. শামসুল আরেফীন",
    "ডা. শামসুল আরেফীন শক্তি",
    "ডেল কার্নেগী",
    "তসলিমা নাসরিন",
    "তাজ হাশমী",
    "তানজিল আরেফীন আদনান",
    "তামিম শাহরিয়ার সুবিন",
    "তারাশঙ্কর বন্দ্যোপাধ্যায়",
    "তাসরিফ খান",
    "তুর্জয় শাকিল",
    "তৃধা আনিকা",
    "তেসাগিগি",
    "তোশিকাযু কাওয়াগুচি",
    "তৌহিদুল ইকবাল সম্পদ",
    "থিবো মেরিস",
    "দিকদর্শন প্রকাশনী",
    "দি নেটওয়ার্ক রিসার্স এন্ড পাবলিকেশন্স",
    "দি রয়েল সায়েন্টিফিক পাবলিকেশন্স",
    "দীপঙ্কর দীপন",
    "নওশীন আহমেদ রোদাল",
    "নাহিদ হাসান মুন্না",
    "নাঈম হোসেন ফারুকী",
    "নাজিম উদ দৌলা",
    "নাবিলা ইষ্ক",
    "নিক পিরোগ",
    "নিশো",
    "নুসরাত সুলতানা সেঁজুতি",
    "নূরে জান্নাত মীম",
    "নেক্সাস পাবলিকেশন্স লিমিটেড",
    "নেপোলিয়ন হিল",
    "নোমান আলী খান",
    "নৌশিন আহমেদ রোদেলা",
    "নসীম হিজাযী",
    "পাওলো কোয়েলহো",
    "পাঞ্জেরী সম্পাদনা পর্ষদ",
    "পানকৌড়ি পাবলিকেশন্স",
    "প্যানাসিয়া পাবলিকেশনস সম্পাদনা পর্ষদ",
    "পিনাকী ভট্টাচার্য",
    "পি সি দাস",
    "প্লাজমিড প্লাস পাবলিকেশন্স",
    "প্রবর রিপন",
    "প্রফেসর’স সম্পাদনা পরিষদ",
    "প্রফেসরস প্রকাশন",
    "প্রাইম পাবলিকেশনস্",
    "প্রিসেপটর্স পাবলিকেশন্স",
    "ফাতেমা তুজ নৌশি",
    "ফাতিহা আয়াত",
    "ফাবিয়াহ্ মমো",
    "ফাহাম আব্দুস সালাম",
    "ফাহমিদ-উর-রহমান",
    "ফারজানা ছবি",
    "ফিকশন",
    "ফিওদোর দস্তয়েভস্কি",
    "ফরিদ আহমেদ",
    "ফরহাদ মজহার",
    "ফ্রান্‌জ কাফকা",
    "ফ্রিল্যান্সার নাসিম",
    "বঙ্কিমচন্দ্র চট্টোপাধ্যায়",
    "বঙ্গবন্ধু শেখ মুজিবুর রহমান",
    "বদরুদ্দীন উমর",
    "বান্না হাসান",
    "বার্ট্রান্ড রাসেল",
    "বাদল সৈয়দ",
    "বিজ্ঞানবিদ্যা টিম",
    "বিভূতিভূষণ বন্দ্যোপাধ্যায়",
    "বিশ্বজিত্ চৌধুরী",
    "বুদ্ধদেব গুহ",
    "বুদ্ধদেব বসু",
    "বেগম রোকেয়া",
    "বেফাকুল মাদারিসিল আরাবিয়া বাংলাদেশ",
    "ব্যতিক্রম প্রকাশনীর সম্পাদনা পরিষদ",
    "ব্রাদার রাহুল হুসাইন (রুহুল আমিন)",
    "ব্রাম স্টোকার",
    "ব্রায়ান ট্রেসি",
    "ব্রিগেডিয়ার জেনারেল (অব:) এম. সাখাওয়াত হোসেন",
    "ভবানী প্রসাদ মজুমদার",
    "ভিক্টর হুগো",
    "মাইকেল মধুসূদন দত্ত",
    "মাইদুল ইসলাম প্রধান (মুকুল)",
    "মাও. মুফতী হিফজুর রহমান",
    "মাওলানা আবু তাহের মেছবাহ",
    "মাওলানা ইসমাইল রেহান",
    "মাওলানা জালাল উদ্দিন রূমী (রহঃ)",
    "মাওলানা জিয়াউল হক",
    "মাওলানা জুলফিকার আহমাদ নকশবন্দি",
    "মাওলানা তারিক জামিল",
    "মাওলানা নাসীম আরাফাত",
    "মাওলানা মুহিউদ্দীন খান",
    "মাওলানা মুহাম্মাদ আবদুর রহীম (রহ)",
    "মাওলানা মুহাম্মাদ আবদুল মালেক",
    "মাওলানা মুহাম্মদ হেমায়েত উদ্দীন",
    "মাওলানা যুলফিকার আহমদ নকশবন্দী",
    "মাশুদুল হক",
    "মাশাহেদ হাসান সীমান্ত",
    "মাসফিক এনাম তূর্য",
    "মাসুদ রানা সাগর",
    "মাসুদ শরীফ",
    "মাকতাবাতুল ফাতাহ বাংলাদেশ",
    "মাকোতো শিনকাই",
    "মানিক বন্দ্যোপাধ্যায়",
    "মাজিদা রিফা",
    "মাহবুব নাহিদ",
    "মাহবুবুর রহমান (তথ্য প্রযুক্তিবিদ)",
    "মাহমুদ বিন নূর",
    "মাহমুদুর রহমান (সাংবাদিক)",
    "মাহিন মাহমুদ",
    "মারজুক রাসেল",
    "মার্ক অনুপম মল্লিক",
    "মিচিও কাকু",
    "মিরাজ রহমান",
    "মির্জা গালিব",
    "মীযান হারুন",
    "মুফাক্কিরে ইসলাম হযরত মাওলানা সাইয়্যিদ আবুল হাসান আলী নদভী রহ.",
    "মুফতি ইসমাঈল মেন্‌ক",
    "মুফতি রেজাউল করীম আবরার",
    "মুন্নি আক্তার প্রিয়া",
    "মুনজেরিন শহীদ",
    "মুনির হাসান",
    "মুযাফফর বিন মুহসিন",
    "মুস্তাফিজ ইবনে আনির",
    "মুহিব খান",
    "মুহম্মদ আনোয়ার হোসেন ফকির",
    "মুহাম্মদ আসাদুল্লাহ আল-গালিব",
    "মুহাম্মদ ইলিয়াস কাঞ্চন (কোচ কাঞ্চন)",
    "মুহাম্মদ জাফর ইকবাল",
    "মুহাম্মদ যাইনুল আবিদীন",
    "মুহাম্মাদ আতীক উল্লাহ",
    "মুহাম্মাদ মুশফিকুর রহমান মিনার",
    "মুসা আল হাফিজ",
    "মেন্টর্স এডুকেশন সিরিজ",
    "মেজর এম এ জলিল",
    "মেজর মোঃ দেলোয়ার হোসেন (ডেল এইচ খান)",
    "মেহেদী হাসান রিফাত",
    "মোজাম্মেল হোসেন ত্বোহা",
    "মোত্তাসিন পাহলভী",
    "মোস্তাক আহ্‌মাদ",
    "মোস্তফা কামাল",
    "মোশতাক আহমেদ",
    "মোঃ আবু বকর সিদ্দিক",
    "মোঃ আল মেহেদী",
    "মোঃ আল-আমিন",
    "মোঃ খাইরুল আলম",
    "মোঃ কেতাব উদ্দীন",
    "মোঃ জাহাঙ্গীর আলম",
    "মোঃ নাঈম হোসেন",
    "মোঃ নূরুল্লাহ হোসাইন",
    "মোঃ ফাহাদ হোসাইন",
    "মোঃ মনিরুল ইসলাম রাজিব",
    "মোঃ মতিউর রহমান",
    "মোঃ মোস্তাফিজুর রহমান মোস্তাক",
    "মোঃ রাশেদ মেনন",
    "মোঃ সজীব আলী",
    "মোঃ সহিদুল ইসলাম রাজন",
    "মো: মাহবুবুল হাসান",
    "মো. টিপু সুলতান",
    "মো. তন্ময় হাসান",
    "মো. নুমেরি সাত্তার অপার",
    "মোহাম্মদ আশিক",
    "মোহাম্মদ নাজিম উদ্দিন",
    "মোহাম্মদ ফারিস",
    "মোহাম্মদ মিরাজ মিয়া",
    "মোহাইমিন পাটোয়ারী",
    "মৌরি মরিয়ম",
    "মম সাহা (বিষাদিনী)",
    "মোমেন তাজোয়ার মমিত",
    "মহিউদ্দিন আহমদ",
    "মহিউদ্দিন মোহাম্মদ",
    "মহাজাতক",
    "যোবায়ের আল মাহমুদ",
    "রউফুল আলম",
    "রকিব হাসান",
    "রকিবুল হাসান",
    "রকমারি",
    "রাগিব হাসান",
    "রাজিব আহমেদ",
    "রাজীব হোসাইন সরকার",
    "রাজীব হোসেন",
    "রাতুল খান",
    "রাফাতুল ইসলাম",
    "রাফান আহমেদ",
    "রাহাত খান",
    "রাহিব",
    "রিসেন্ট পাবলিকেশন এডিটরিয়াল বোর্ড",
    "রুমানা বৈশাখী",
    "রয়েল সম্পাদনা পর্ষদ",
    "রবার্ট গ্রীনে",
    "রবার্ট টি. কিয়োসাকি",
    "রবিউল আলম লুইপা",
    "রবিন জামান খান",
    "রবীন্দ্রনাথ ঠাকুর",
    "রোকসানা আক্তার রুপী",
    "লেইল লোনডেস",
    "লেকচার",
    "লতিফুল ইসলাম শিবলী",
    "লস্ট মডেস্টি",
    "লুৎফুল কায়সার",
    "লিও তলস্তয়",
    "শওকত আলী",
    "শরৎচন্দ্র চট্টোপাধ্যায়",
    "শরদিন্দু বন্দ্যোপাধ্যায়",
    "শরিফুল ইসলাম আলাল",
    "শরিফুল হক ডালিম (বীর উত্তম)",
    "শরীফ মুহাম্মাদ ইউনুস",
    "শরীফুল হাসান",
    "শর্মিলা বোস",
    "শহীদুল জহির",
    "শাইখ আহমাদ মুসা জিবরিল",
    "শাইখ আহম্মেদ হুসাইন দিদাত",
    "শাইখ আব্দুল মালিক আল কাসিম",
    "শাইখ আব্দুল হামীদ আল-ফাইযী আল-মাদানী",
    "শাইখ খালিদ আর রশিদ",
    "শাইখ ড. তাওফিক চৌধুরি",
    "শাইখ মুহাম্মাদ সালেহ আল মুনাজ্জিদ",
    "শাইখ মূসা জিবরীল",
    "শাইখ সাঈদ ইবনে আলী ইবনে ওয়াহফি আলকাহতানী",
    "শাইখুল ইসলাম মুফতী মুহাম্মাদ তাকী উসমানী",
    "শাইখুল হাদীস মাওলানা যাকারিয়া রহ.",
    "শানজানা আলম",
    "শাব্বির আহসান",
    "শামসুল হক",
    "শামিমা শ্রাবনী",
    "শামসুর রাহমান",
    "শারমিন আহমদ",
    "শাহাদুজ্জামান",
    "শাহাদাৎ হুসাইন খান ফয়সাল",
    "শাহরিয়ার",
    "শাহরোজ ফারদি",
    "শায়খ আহমাদুল্লাহ",
    "শায়খ আব্দুল ফাত্তাহ আবু গুদ্দাহ রহ.",
    "শায়খ মোখতার আহমাদ",
    "শায়খ মুফতি মুশতাকুন্নবী হাফিজাহুল্লাহ",
    "শশাঙ্ক",
    "শীর্ষেন্দু মুখোপাধ্যায়",
    "শেখ হাসিনা",
    "ষোলো টীম",
    "সংশপ্তক পাবলিকেশন্স",
    "সদর উদ্দিন আহমদ চিশতী",
    "সদগুরু",
    "সত্যজিৎ রায়",
    "সন্দীপন",
    "সনাতন গোস্বামী",
    "সঞ্জিত কুমার গুহ",
    "সমকালীন সংকলন টিম",
    "সমরেশ মজুমদার",
    "সরদার ফজলুল করিম",
    "সলোমন নর্থআপ",
    "সালমা চৌধুরী",
    "সালমান হক",
    "সালমান হাবীব (কবিতায় গল্প বলা মানুষ)",
    "সাতোশি ইয়াগিসাওয়া",
    "সাদাত হোসাইন",
    "সাদিয়া খান সুবাসিনী",
    "সাদমান সাদিক",
    "সানজিদা বিনতে সফি",
    "সাবিত রায়হান",
    "সামিয়া খান প্রিয়া",
    "সায়ীদ উসমান",
    "সাইফুর রহমান খান",
    "সাইফুল ইসলাম",
    "সাইফুল হোসেন",
    "সাইয়েদ আবুল আলা মওদূদী",
    "সাইয়েদ কুতুব শহীদ",
    "সাইয়্যেদ সুলাইমান নদভি (রহ.)",
    "সিদ্দিক মহসিন পাটোয়ারী",
    "সিহিন্তা শরীফা",
    "সিরাজুল ইসলাম চৌধুরী",
    "সুকান্ত ভট্টাচার্য",
    "সুকুমার রায়",
    "সুনীল গঙ্গোপাধ্যায় (নীললোহিত)",
    "সুমন্ত আসলাম",
    "সেলিনা হোসেন",
    "সেলিম টি এস আল-হাসসানি",
    "সৈয়দ ওয়ালীউল্লাহ্‌",
    "সৈয়দ মুজতবা আলী",
    "সৈয়দ শামসুল হক",
    "সৈয়দ সাকিব আলম",
    "সলিমুল্লাহ খান",
    "সৌমিত্র চক্রবর্তী",
    "স্যার আর্থার কোনান ডয়েল",
    "স্বপ্নীল চক্রবর্ত্তী",
    "সবুজ আহম্মদ মুরসালিন",
    "সাহাদত হোসেন খান",
    "স্টিফেন আর. কোভি",
    "স্টিফেন কিং",
    "স্টিফেন হকিং",
    "হাকীমুল উম্মত মাওলানা আশরাফ আলী থানভী রহ.",
    "হাসান ইথার",
    "হাসান জামান",
    "হান কাং",
    "হামিদা মুবাশ্বেরা",
    "হায়াৎ মামুদ",
    "হারুকি মুরাকামি",
    "হালিম",
    "হুমায়ূন আহমেদ",
    "হুজ্জাতুল ইসলাম ইমাম গাযযালী রহ.",
    "হেলাল হাফিজ",
    "হেমিন সুনিম",
    "হেমেন্দ্রকুমার রায়",
    "হেমায়েত মাতুব্বর",
    "হেনরি ফোর্ড",
    "হেনরি রাইডার হ্যাগার্ড",
    "হযরত মাওলানা মুফতী মুহাম্মদ শফী (রহ.)",
    "হোসাইন-এ-তানভীর",
    "হরিশংকর জলদাস"
];

export const EDUCATONAL_INSTITUTE = [
    // --- Public Universities ---
    "Bangabandhu Sheikh Mujib Medical University (BSMMU)",
    "Bangabandhu Sheikh Mujibur Rahman Agricultural University (BSMRAAU)",
    "Bangabandhu Sheikh Mujibur Rahman Aviation and Aerospace University (BSMRAAU)",
    "Bangabandhu Sheikh Mujibur Rahman Digital University, Bangladesh (BDU)",
    "Bangabandhu Sheikh Mujibur Rahman Maritime University (BSMRMU)",
    "Bangabandhu Sheikh Mujibur Rahman Science and Technology University (BSMRSTU), Gopalganj",
    "Bangabandhu Sheikh Mujibur Rahman Science and Technology University, Pirojpur",
    "Bangabandhu Sheikh Mujibur Rahman University, Kishoreganj",
    "Bangamata Sheikh Fojilatunnesa Mujib Science and Technology University (BSFMSTU)",
    "Bangladesh Agricultural University (BAU)",
    "Bangladesh Army University of Engineering and Technology (BAUET)",
    "Bangladesh Army University of Science and Technology (BAUST)",
    "Bangladesh Open University (BOU)",
    "Bangladesh University of Engineering and Technology (BUET)",
    "Bangladesh University of Professionals (BUP)",
    "Bangladesh University of Textiles (BUTEX)",
    "Barishal University (BU)",
    "Begum Rokeya University, Rangpur (BRUR)",
    "Chandpur Science and Technology University",
    "Chittagong Medical University (CMU)",
    "Chittagong University of Engineering and Technology (CUET)",
    "Chittagong Veterinary and Animal Sciences University (CVASU)",
    "Comilla University (CoU)",
    "Dhaka University of Engineering and Technology (DUET)",
    "Habiganj Agricultural University",
    "Hajee Mohammad Danesh Science and Technology University (HSTU)",
    "Islamic Arabic University",
    "Islamic University (IU), Kushtia",
    "Jagannath University (JnU)",
    "Jahangirnagar University (JU)",
    "Jamalpur Science and Technology University",
    "Jashore University of Science and Technology (JUST)",
    "Jatiya Kabi Kazi Nazrul Islam University (JKKNIU)",
    "Khulna Agricultural University",
    "Khulna University (KU)",
    "Khulna University of Engineering and Technology (KUET)",
    "Kurigram Agricultural University",
    "Mawlana Bhashani Science and Technology University (MBSTU)",
    "Mujibnagar University, Meherpur",
    "Naogaon University",
    "National University (NU)",
    "Netrokona University",
    "Noakhali Science and Technology University (NSTU)",
    "Pabna University of Science and Technology (PUST)",
    "Patuakhali Science and Technology University (PSTU)",
    "Rabindra University, Bangladesh",
    "Rajshahi Medical University (RMU)",
    "Rajshahi University of Engineering and Technology (RUET)",
    "Rangamati Science and Technology University (RMSTU)",
    "Shahjalal University of Science and Technology (SUST)",
    "Sheikh Hasina University",
    "Sher-e-Bangla Agricultural University (SAU)",
    "Sunamganj Science and Technology University",
    "Sylhet Agricultural University (SAU)",
    "Sylhet Medical University",
    "University of Chittagong (CU)",
    "University of Dhaka (DU)",
    "University of Rajshahi (RU)",

    // --- Private Universities ---
    "Ahsanullah University of Science and Technology (AUST)",
    "American International University-Bangladesh (AIUB)",
    "Anwer Khan Modern University",
    "ASA University Bangladesh",
    "Asian University of Bangladesh (AUB)",
    "Atish Dipankar University of Science and Technology (ADUST)",
    "Bandarban University",
    "Bangabandhu Sheikh Mujibur Rahman University, Naogaon",
    "Bangladesh Army International University of Science and Technology (BAIUST)",
    "Bangladesh Islami University (BIU)",
    "Bangladesh University (BU)",
    "Bangladesh University of Business and Technology (BUBT)",
    "Bangladesh University of Health Sciences (BUHS)",
    "BGC Trust University Bangladesh",
    "BGMEA University of Fashion and Technology (BUFT)",
    "BRAC University",
    "Britannia University",
    "Canadian University of Bangladesh",
    "CCN University of Science and Technology",
    "Central University of Science and Technology (CUST)",
    "Central Women's University (CWU)",
    "Chittagong Independent University (CIU)",
    "City University (CU)",
    "Cox's Bazar International University",
    "Daffodil International University (DIU)",
    "Dhaka International University (DIU)",
    "East Delta University (EDU)",
    "East West University (EWU)",
    "Eastern University (EU)",
    "European University of Bangladesh (EUB)",
    "Exim Bank Agricultural University, Bangladesh (EBAUB)",
    "Fareast International University (FIU)",
    "Feni University",
    "First Capital University of Bangladesh",
    "German University Bangladesh",
    "Global University Bangladesh",
    "Gono Bishwabidyalay",
    "Green University of Bangladesh (GUB)",
    "Hamdard University Bangladesh",
    "IBAIS University",
    "Independent University, Bangladesh (IUB)",
    "International Islamic University Chittagong (IIUC)",
    "International Standard University (ISU)",
    "International University of Business Agriculture and Technology (IUBAT)",
    "Ishakha International University, Bangladesh",
    "Khulna Khan Bahadur Ahsanullah University",
    "Khwaja Yunus Ali University (KYAU)",
    "Lalon University of Science and Arts",
    "Leading University (LU)",
    "Manarat International University (MIU)",
    "Metropolitan University (MU)",
    "Microland University of Science and Technology",
    "N.P.I University of Bangladesh",
    "North Bengal International University (NBIU)",
    "North East University Bangladesh (NEUB)",
    "North South University (NSU)",
    "North Western University (NWU)",
    "Northern University Bangladesh (NUB)",
    "Northern University of Business and Technology, Khulna",
    "Notre Dame University Bangladesh (NDUB)",
    "Port City International University",
    "Premier University",
    "Presidency University",
    "Prime University",
    "Primeasia University",
    "Pundra University of Science and Technology",
    "Queens University",
    "R.P. Shaha University",
    "Rabindra Maitree University",
    "Rajshahi Science and Technology University (RSTU)",
    "Ranada Prasad Shaha University",
    "Royal University of Dhaka",
    "Rupayan A.K.M Shamsuzzoha University",
    "Shanto-Mariam University of Creative Technology (SMUCT)",
    "Sheikh Fazilatunnesa Mujib University",
    "Sonargaon University (SU)",
    "Southeast University (SEU)",
    "Southern University Bangladesh",
    "Stamford University Bangladesh",
    "State University of Bangladesh (SUB)",
    "Sylhet International University (SIU)",
    "Tagore University of Creative Arts",
    "The International University of Scholars",
    "The Millennium University (TMU)",
    "The People's University of Bangladesh (PUB)",
    "Times University, Bangladesh",
    "Trust University, Barishal",
    "United International University (UIU)",
    "University of Asia Pacific (UAP)",
    "University of Brahmanbaria",
    "University of Creative Technology, Chittagong",
    "University of Development Alternative (UODA)",
    "University of Global Village (UGV)",
    "University of Information Technology and Sciences (UITS)",
    "University of Liberal Arts Bangladesh (ULAB)",
    "University of Science and Technology Chittagong (USTC)",
    "University of Skill Enrichment and Technology (USET)",
    "University of South Asia (USA)",
    "Uttara University (UU)",
    "Varendra University (VU)",
    "Victoria University of Bangladesh (VUB)",
    "World University of Bangladesh (WUB)",
    "Z.H Sikder University of Science and Technology",
    "ZNRF University of Management Sciences",

    // --- International Universities ---
    "Asian University for Women (AUW)",
    "Islamic University of Technology (IUT)",

    // --- Medical Universities ---
    "Bangabandhu Sheikh Mujib Medical University (BSMMU), Dhaka",
    "Chittagong Medical University (CMU), Chittagong",
    "Rajshahi Medical University (RMU), Rajshahi",
    "Sylhet Medical University (SMU), Sylhet",
    "Sheikh Hasina Medical University, Khulna",

    // --- Government Medical Colleges ---
    "Abdul Malek Ukil Medical College, Noakhali",
    "Bangabandhu Medical College, Sunamganj",
    "Bangabandhu Sheikh Mujib Medical College, Faridpur",
    "Barishal Sher-e-Bangla Medical College (SBMC), Barishal",
    "Chandpur Medical College, Chandpur",
    "Chattogram Medical College (CMC), Chattogram",
    "Colonel Malek Medical College, Manikganj",
    "Comilla Medical College, Comilla",
    "Cox's Bazar Medical College, Cox's Bazar",
    "Dhaka Medical College (DMC), Dhaka",
    "Dinajpur Medical College, Dinajpur",
    "Gopalganj Sheikh Sayera Khatun Medical College, Gopalganj",
    "Jashore Medical College, Jashore",
    "Khulna Medical College (KMC), Khulna",
    "Kushtia Medical College, Kushtia",
    "M. Abdur Rahim Medical College, Dinajpur",
    "Magura Medical College, Magura",
    "Mugda Medical College, Dhaka",
    "Mymensingh Medical College (MMC), Mymensingh",
    "Naogaon Medical College, Naogaon",
    "Netrokona Medical College, Netrokona",
    "Nilphamari Medical College, Nilphamari",
    "Noakhali Medical College, Noakhali",
    "Pabna Medical College, Pabna",
    "Patuakhali Medical College, Patuakhali",
    "Rajshahi Medical College (RMC), Rajshahi",
    "Rangamati Medical College, Rangamati",
    "Rangpur Medical College, Rangpur",
    "Satkhira Medical College, Satkhira",
    "Shaheed M. Monsur Ali Medical College, Sirajganj",
    "Shaheed Suhrawardy Medical College, Dhaka",
    "Shaheed Syed Nazrul Islam Medical College, Kishoreganj",
    "Shaheed Tajuddin Ahmad Medical College, Gazipur",
    "Shaheed Ziaur Rahman Medical College, Bogura",
    "Sheikh Hasina Medical College, Habiganj",
    "Sheikh Hasina Medical College, Jamalpur",
    "Sheikh Hasina Medical College, Tangail",
    "Sher-e-Bangla Medical College, Barisal",
    "Sir Salimullah Medical College (SSMC), Dhaka",
    "Sylhet M.A.G. Osmani Medical College (SOMC), Sylhet",

    // --- Army & Defense Medical Colleges ---
    "Armed Forces Medical College (AFMC), Dhaka",
    "Army Medical College, Bogura",
    "Army Medical College, Chattogram",
    "Army Medical College, Comilla",
    "Army Medical College, Jashore",
    "Army Medical College, Rangpur",

    // --- Private Medical Colleges ---
    "Ad-din Sakina Medical College, Jashore",
    "Ad-din Women's Medical College, Dhaka",
    "Ahsania Mission Medical College, Dhaka",
    "Aichi Medical College, Dhaka",
    "Anwer Khan Modern Medical College, Dhaka",
    "Ashiyan Medical College, Dhaka",
    "B.G.C. Trust Medical College, Chattogram",
    "Bangladesh Medical College, Dhaka",
    "Barind Medical College, Rajshahi",
    "Bashundhara Ad-din Medical College, Dhaka",
    "Bikrampur Bhuiyan Medical College, Munshiganj",
    "Brahmanbaria Medical College, Brahmanbaria",
    "Care Medical College, Dhaka",
    "Central Medical College, Comilla",
    "Chattogram International Medical College, Chattogram",
    "Chattogram Maa-O-Shishu Hospital Medical College, Chattogram",
    "City Medical College, Gazipur",
    "Community Based Medical College, Mymensingh",
    "Cox's Bazar International Medical College, Cox's Bazar",
    "Delta Medical College, Dhaka",
    "Dhaka Central International Medical College, Dhaka",
    "Dhaka Community Medical College, Dhaka",
    "Dhaka National Medical College, Dhaka",
    "Diabetic Association Medical College, Faridpur",
    "Dr. Sirajul Islam Medical College, Dhaka",
    "East West Medical College, Dhaka",
    "Eastern Medical College, Comilla",
    "Enam Medical College, Savar, Dhaka",
    "Gazi Medical College, Khulna",
    "Gonoshasthaya Samaj Vittik Medical College, Savar",
    "Green Life Medical College, Dhaka",
    "Holy Family Red Crescent Medical College, Dhaka",
    "Ibn Sina Medical College, Dhaka",
    "Institute of Applied Health Sciences (IAHS) Medical College, Chattogram",
    "International Medical College, Gazipur",
    "Islami Bank Medical College, Rajshahi",
    "Jahurul Islam Medical College, Kishoreganj",
    "Jalalabad Ragib-Rabeya Medical College, Sylhet",
    "Khawaja Yunus Ali Medical College, Sirajganj",
    "Khulna City Medical College, Khulna",
    "Kumudini Women's Medical College, Tangail",
    "Labaid Medical College, Dhaka",
    "Mainamati Medical College, Comilla",
    "Marks Medical College, Dhaka",
    "Medical College for Women and Hospital, Dhaka",
    "MH Samorita Medical College, Dhaka",
    "Monno Medical College, Manikganj",
    "Munnu Medical College, Manikganj",
    "Nightingale Medical College, Dhaka",
    "North Bengal Medical College, Sirajganj",
    "North East Medical College, Sylhet",
    "Northern International Medical College, Dhaka",
    "Parkview Medical College, Sylhet",
    "Popular Medical College, Dhaka",
    "Prime Medical College, Rangpur",
    "Rangpur Community Medical College, Rangpur",
    "Sahabuddin Medical College, Dhaka",
    "Samorita Medical College, Dhaka",
    "Shahabuddin Medical College, Dhaka",
    "Shaheed Monsur Ali Medical College, Dhaka",
    "Southern Medical College, Chattogram",
    "Sylhet Women's Medical College, Sylhet",
    "Tairunnessa Memorial Medical College, Gazipur",
    "TMSS Medical College, Bogura",
    "United Medical College, Dhaka",
    "Universal Medical College, Dhaka",
    "US-Bangla Medical College, Narayanganj",
    "Uttara Adhunik Medical College, Dhaka",
    "Z.H. Sikder Women's Medical College, Dhaka",

    // --- Cadet Colleges ---
    "Barishal Cadet College (BCC)",
    "Comilla Cadet College (CCC)",
    "Faujdarhat Cadet College (FCC)",
    "Jhenaidah Cadet College (JCC)",
    "Mirzapur Cadet College (MCC)",
    "Pabna Cadet College (PCC)",
    "Rajshahi Cadet College (RCC)",
    "Rangpur Cadet College (RCC)",
    "Sylhet Cadet College (SCC)",
    "Feni Girls' Cadet College (FGCC)",
    "Joypurhat Girls' Cadet College (JGCC)",
    "Mymensingh Girls' Cadet College (MGCC)",

    // --- Colleges (Division Wise) ---
    "Dhaka College, Dhaka",
    "Eden Mohila College, Dhaka",
    "Notre Dame College, Dhaka",
    "Viqarunnisa Noon School & College, Dhaka",
    "Rajuk Uttara Model College, Dhaka",
    "Holy Cross College, Dhaka",
    "Dhaka Residential Model College, Dhaka",
    "St. Joseph Higher Secondary School, Dhaka",
    "Government Titumir College, Dhaka",
    "Government Bangla College, Dhaka",
    "Kabi Nazrul Government College, Dhaka",
    "Government Shaheed Suhrawardy College, Dhaka",
    "Begum Badrunnesa Government Girls' College, Dhaka",
    "Dhaka City College, Dhaka",
    "Dhaka Commerce College, Dhaka",
    "Adamjee Cantonment College, Dhaka",
    "BAF Shaheen College, Dhaka",
    "Tejgaon College, Dhaka",
    "Siddheswari Girls' College, Dhaka",
    "Lalmatia Mohila College, Dhaka",
    "Government Tolaram College, Narayanganj",
    "Government Haraganga College, Munshiganj",
    "Government Debendra College, Manikganj",
    "Government Rajendra College, Faridpur",
    "Gurudayal Government College, Kishoreganj",
    "Narsingdi Government College, Narsingdi",
    "Bhawal Badre Alam Government College, Gazipur",
    "Tongi Government College, Gazipur",
    "Chittagong College, Chattogram",
    "Government Hazi Mohammad Mohsin College, Chattogram",
    "Government City College, Chattogram",
    "Chittagong Government Women's College, Chattogram",
    "Omargani M.E.S. College, Chattogram",
    "Ispahani Public School & College, Chattogram",
    "Chittagong Cantonment Public College, Chattogram",
    "BAF Shaheen College, Chattogram",
    "Patiya Government College, Chattogram",
    "Cox's Bazar Government College, Cox's Bazar",
    "Comilla Victoria Government College, Comilla",
    "Comilla Government College, Comilla",
    "Ibn Taimiya School and College, Comilla",
    "Brahmanbaria Government College, Brahmanbaria",
    "Chandpur Government College, Chandpur",
    "Noakhali Government College, Noakhali",
    "Chowmuhani Government S.A. College, Noakhali",
    "Feni Government College, Feni",
    "Lakshmipur Government College, Lakshmipur",
    "Rajshahi College, Rajshahi",
    "New Government Degree College, Rajshahi",
    "Rajshahi Government City College, Rajshahi",
    "Rajshahi Government Women's College, Rajshahi",
    "Government Edward College, Pabna",
    "Government Azizul Haque College, Bogura",
    "Bogura Cantonment Public School & College, Bogura",
    "Joypurhat Government College, Joypurhat",
    "Naogaon Government College, Naogaon",
    "Nawabganj Government College, Chapainawabganj",
    "Sirajganj Government College, Sirajganj",
    "N.S. Government College, Natore",
    "Government B.L. College, Khulna",
    "Government Majid Memorial City College, Khulna",
    "Government Pioneer Women's College, Khulna",
    "Azam Khan Government Commerce College, Khulna",
    "Khulna Public College, Khulna",
    "Government M.M. College, Jashore",
    "Cantonment College, Jashore",
    "Kushtia Government College, Kushtia",
    "Government P.C. College, Bagerhat",
    "Satkhira Government College, Satkhira",
    "Chuadanga Government College, Chuadanga",
    "Meherpur Government College, Meherpur",
    "Narail Government Victoria College, Narail",
    "Magura Government College, Magura",
    "Government Brojomohun (BM) College, Barishal",
    "Government Syed Hatem Ali College, Barishal",
    "Government Barishal College, Barishal",
    "Amrita Lal Dey College, Barishal",
    "Government Women's College, Barishal",
    "Bhola Government College, Bhola",
    "Patuakhali Government College, Patuakhali",
    "Barguna Government College, Barguna",
    "Pirojpur Government Suhrawardy College, Pirojpur",
    "Jhalakathi Government College, Jhalakathi",
    "Murari Chand (MC) College, Sylhet",
    "Sylhet Government College, Sylhet",
    "Sylhet Government Women's College, Sylhet",
    "Madan Mohan College, Sylhet",
    "Jalalabad Cantonment Public School and College, Sylhet",
    "Brindaban Government College, Habiganj",
    "Moulvibazar Government College, Moulvibazar",
    "Sunamganj Government College, Sunamganj",
    "Carmichael College, Rangpur",
    "Rangpur Government College, Rangpur",
    "Government Begum Rokeya College, Rangpur",
    "Cantonment Public School & College, Rangpur",
    "Dinajpur Government College, Dinajpur",
    "Kurigram Government College, Kurigram",
    "Gaibandha Government College, Gaibandha",
    "Nilphamari Government College, Nilphamari",
    "Panchagarh M.R. Government College, Panchagarh",
    "Thakurgaon Government College, Thakurgaon",
    "Lalmonirhat Government College, Lalmonirhat",
    "Ananda Mohan College, Mymensingh",
    "Shahid Syed Nazrul Islam College, Mymensingh",
    "Muminunnisa Government Women's College, Mymensingh",
    "Cantonment Public School and College, Momenshahi",
    "Nasirabad College, Mymensingh",
    "Notre Dame College, Mymensingh",
    "Netrokona Government College, Netrokona",
    "Sherpur Government College, Sherpur",
    "Jamalpur Government Asheq Mahmud College, Jamalpur"
];